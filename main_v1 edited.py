...
# ✅ LETAKKAN DI SINI - Setelah model, sebelum endpoint
async def scan_for_device(mac_address: str, max_retries: int = 3, timeout: float = 10.0):
    """Scan device dengan retry"""
    for attempt in range(max_retries):
        ...
        if selected_device:
            print(f"Device found: {selected_device.address}")
            return selected_device
        
        if attempt < max_retries - 1:
            print(f"Device not found, retrying in 5 seconds...")
...
async def connect_and_read(data: ConnectAndReadInput):
    """
    Read data dari device yang SUDAH ter-pair.
    """
    try:
        # Gunakan scanning dengan retry
        selected_device = await scan_for_device(data.mac_address, max_retries=3, timeout=10.0)
        
        if not selected_device:
            raise HTTPException(
                status_code=404, 
                detail=f"Device {data.mac_address} not found after multiple scan attempts. "
                       "Please ensure device is powered on and in range."
            )
        
        client = BleakClient(selected_device.address)
        try:
            print(f"Connecting to {selected_device.address}...")
            await client.connect()
            ...
            if client.is_connected:
                print("Disconnecting...")
                await client.disconnect()
                await asyncio.sleep(3)  # Tambahkan delay lebih lama
                print("Disconnected")
